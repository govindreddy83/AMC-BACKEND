...
class PlannerDetailService {
  ...
  static async getPlannerByNo(plannerNo) {
    ...
    if (rawRows && rawRows.length > 1) {
      ...
      const serialNoIdx = findHeaderIndex(['serial no', 'serial_no', 'serialno', 'serial number', 'serial_number', 'sl.no', 's.no', 'sno', 's. no', 'sl no']);
      ...
      if (plannerNoIdx !== -1) {
        ...
        for (let i = startIdx; i < rawRows.length; i++) {
          ...
          if (cellVal === targetNo) {
            ...
            const historyLogs = PlannerDetailService.extractDynamicHistoryLogs(rawRows[0], rawRows[i], currentPlannerNo, allMappings);
  ...
  static extractDynamicHistoryLogs(rawHeaders, rowData, plannerNo = '', allMappings = {}) {
    const logs = [];
  ...
  static async getAllPlanners() {
    ...
    if (rawRows && rawRows.length > 1) {
      ...
      if (plannerNoIdx !== -1) {
        ...
        for (let i = startIdx; i < rawRows.length; i++) {
          const plannerNo = rawRows[i][plannerNoIdx] ? rawRows[i][plannerNoIdx].toString().trim() : '';
          if (!plannerNo) continue;
          ...
          const historyLogs = PlannerDetailService.extractDynamicHistoryLogs(rawRows[0], rawRows[i], plannerNo, allMappings);
